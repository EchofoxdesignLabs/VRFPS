...
namespace VRFPSKit
{
    ...
    public class StickyWeaponSocket : MonoBehaviour
    {
        ...
        private void OnHandReleaseObject(SelectExitEventArgs selectExitEvent)
        {
            ...
            if (!releasedInteractable.IsSelectableBy(_socket) || !_socket.CanSelect(releasedInteractable))
                return;
            if (GetComponentInParent<Damageable>().health < 0) return;//Don't reattach to dead player 
