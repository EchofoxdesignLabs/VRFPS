Shader "VRDefender/Sketch"
{
		...
		Pass
    {
        ...
        {
            // We no longer need to sample the depth texture here.
            ...
            for (int x = lower; x <= upper; x += _BlurStepSize)
            {
                ...
                // We sample the .r channel because the shadow map is a single-channel texture.
                col += gauss * SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp, uv).r;
            }

            // Avoid division by zero if kernelSum is 0
            if (kernelSum > 0.0f)
                col /= kernelSum;
		...
		// Pass 2: Vertical Blur (Corrected)
    Pass
    {
        Name "Vertical Blur"
        HLSLPROGRAM
